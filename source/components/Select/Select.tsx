...
class Select extends React.Component<SelectProps, SelectState> {
  ...
  static propTypes = {
    ...
    maxLabelClearPanelHeight: PropTypes.oneOfType([
      PropTypes.string,
      PropTypes.number
    ]),
      ...
      if (valueType === "array") {
        return (
          (value &&
            value.map(obj => {
              const option =
                optionList.find(option => option.key === obj.key) || {};
        ...
        const option =
          optionList.find(option => option.key === value.key) || {};
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    if (
      ...
      !isEqual(nextProps.visible, prevProps.visible)
    ) {
  ...
  constructor(props) {
    ...
    const initialSelectValue = Select.getValueFromProps(
      initialValue,
      labelInValue,
      children
    );
  ...
  focusEvent = event => {
    ...
    if (targetElement) {
      ...
      setTimeout(() => {
        const targetElement = showSearch
          ? this.selectSearch.searchInput.input
          : this.selection;
  ...
  onOptionClick = (e, obj, clickInLabel) => {
    ...
    if (mode === "single") {
      ...
      if (clickInLabel) {
        ...
        const firstValue = selectValueForMultiplePanel.slice(
          0,
          indexInMultiple
        );
        const restValues = selectValueForMultiplePanel.slice(
          indexInMultiple + 1
        );
    ...
    if (isMultiplePanel) {
      return (
        ...
        optionList.every(selected => {
          return !!selectValueForMultiplePanel.find(
  ...
  handleKeyboardEvent = e => {
    ...
    if (
      ...
      if (keyCode === KeyCode.ENTER) {
        const activeTabIndex = optionList.findIndex(
              ...
              this.setState(
                ...
                () => {
                  if (labelInValue) {
                    ...
                    onChange(
                      this.state.selectValue.map(selected => selected.key)[0]
                    );
                  }
                }
              );
  ...
  getDropdownPanel() {
    ...
    const showNotFoundContent = !Select.getOptionFromChildren(
      optionFilteredList
    ).length; // optionList为空判断
    const maxCountError =
      "maxCount" in this.props && selectValue.length > maxCount; // maxCount值存在且小于选择数量
    const requiredError =
      mode === "multiple" && required && !selectValue.length; // required模式下，必须要有option选择
        ...
        {loading ? (
          ...
          <div className={`${dropdownCls}-content`}>
            ...
            >
              ...
              {//列表及空状态框
              showNotFoundContent ? (
                <div className={`${dropdownCls}-not-found`}>
                  {notFoundContent}
                </div>
              ) : (
                <div className={`${dropdownCls}-filtered-list`}>
                  {optionFilteredList}
                </div>
              )}
            </div>
              ...
              <div>
                {maxCountError && (
                  <div className={`${dropdownCls}-error-panel`}>
                    <p className={`${dropdownCls}-error-panel-msg`}>
                      {errorMessage}
                    </p>
                  </div>
                )}
          ...
          <div className={`${selectionCls}-content`}>
            ...
            showArrow && !labelClear && (
              <div className={`${selectionCls}-caret`}>
                <Icon
                  type="down-fill"
                  className={classNames({ ["open"]: popupVisible })}
                />
              </div>
            )}
            ...
            mode === "multiple" &&
              (labelClear ? (
                ...
                selectValueForMultiplePanel &&
                selectValueForMultiplePanel.length ? (
                  <Animate
                    onEnd={this.resizeTrigger}
                    component="div"
                    transitionName="zoom"
                    style={{
                      maxHeight: maxLabelClearPanelHeight
                        ? maxLabelClearPanelHeight
                        : null
                    }}
