...
const CON_MAX_WIDTH =
  conMaxWidth > window.innerWidth ? window.innerWidth : conMaxWidth; //容器最大宽度
const CON_MIN_WIDTH = 360; //容器最小宽度
const CON_MAX_HEIGHT =
  conMaxHeight > window.innerHeight ? window.innerHeight : conMaxHeight; //容器最大高度
...
function isLargger(el1, el2) {
  return (
    el1.clientHeight > el2.clientHeight || el1.clientWidth > el2.clientWidth
  );
}
...
> {
  ...
  setContainerStyle = () => {
    ...
    getImageSize(this.imgEl, (naturalWidth, naturalHeight) => {
      ...
      if (!this.state.shown) {
        ...
        if (!this.props.mask) {
          css.left = num2px(
            (window.innerWidth - width) / 2 + window.pageXOffset
          );
          css.top = num2px(
            (window.innerHeight - height) / 2 + window.pageYOffset
          );
        }
  ...
  handleMouseDown = e => {
    ...
    if (
      tar === this.imgEl &&
      (this.state.container.isFull || isLargger(this.imgEl, this.$el))
    ) {
  ...
  handleWheel = e => {
    ...
    this.handleZoom(
      this.state.image.ratio + (delta > 0 ? STEP_RATIO : -STEP_RATIO)
    );
  };
  ...
  render() {
    ...
    let isHide = !(source.length > 1 || (!!children && (children as React.ReactNodeArray).length > 1));
    ...
    const renderCtner = (
      ...
      >
        ...
        <div
          ...
          style={isFullscreen ? { bottom: "20px" } : null}
        >
