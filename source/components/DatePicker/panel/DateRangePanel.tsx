...
import { BasePickerProps, Mode } from '../BasePicker';
...
const isInputValid = (text, date, disabledDate) => {
  if (
    text.trim() === '' ||
    !isValidValueArr(date) ||
    !DateRangePanel.isValid(date, disabledDate)
  )
    return false;
  return true;
};
...
const dateToStr = date => {
  ...
  const tdate = date;
  const formatter = TYPE_VALUE_RESOLVER_MAP['timeselect'].formatter;
...
interface DateRangePanelState {
  ...
  prevPropValue?: DateRangePanelState;
  rangeState?: any;
}
...
> {
  static isValid = (value: string[], disabledDate) => {
    ...
    return typeof disabledDate === 'function' && value && value.length >= 2
  ...
  static propsToState(props) {
    ...
    const minTime = isValidValueArr(props.value)
      ? toDate(props.value[0])
      : toDate(props.defaultStartTimeValue);
    const maxTime = isValidValueArr(props.value)
      ? toDate(props.value[1])
      : toDate(props.defaultEndTimeValue);
    const minDate = isValidValueArr(props.value)
      ? toDate(props.value[0])
      : null;
    const maxDate = isValidValueArr(props.value)
      ? toDate(props.value[1])
      : null;
    ...
    const state: DateRangePanelState = {};
    // 左侧日历月份
    ...
    // 开始日期输入框中的值
    state.minDateInputText = isValidValueArr(props.value)
      ? formatDate(props.value[0], dateFormat(props.format))
      : '';
    // 结束日期输入框中的值
    state.maxDateInputText = isValidValueArr(props.value)
      ? formatDate(props.value[1], dateFormat(props.format))
      : '';
    // 开始时间
  ...
  static getEndTimeSelectableRange = ({
    ...
    if (endTimeSelectMode === 'TimePicker') {
      const propsTimeRangeArr = converSelectRange({
      ...
      if (propsTimeRangeArr && propsTimeRangeArr.length > 0) {
        ...
        for (let range of propsTimeRangeArr) {
          min = new Date(Math.max(min.valueOf(), range[0]));
          max = new Date(Math.min(max.valueOf(), range[1]));
          rangeResult.push(
            `${getTimeString(new Date(min))} - ${getTimeString(new Date(max))}`,
          );
        }
      } else {
      ...
      let propsResult = Object.assign({}, endTimeSelectModeProps, {
        minTime: dateToStr(minTime),
      });
      return propsResult;
    }
  };
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    if (
      'value' in nextProps &&
      !isEqual(prevState.prevPropValue, nextProps.value)
    ) {
  ...
  handleChangeRange(rangeState) {
    this.setState({
      minDate: new Date(
        Math.min(rangeState.firstSelectedValue, rangeState.endDate),
      ),
      maxDate: new Date(
        Math.max(rangeState.firstSelectedValue, rangeState.endDate),
      ),
    });
  }
    ...
    if (endTimeSelectMode === 'TimePicker') {
      return maxTime &&
        TimePanel.isValid(
          maxTime,
          converSelectRange({
            selectableRange: this.getEndTimeSelectableRange(minTime),
          }),
        )
        ? maxTime
        : null;
  ...
  setEndTimeRange = () => {
    ...
    this.setState({
      maxTime:
        isSameDate && minTime ? this.getMaxTime(maxTime, minTime) : maxTime,
      endTimeSelectableRange:
  ...
  handleDateInputChange(e, type) {
    ...
    if (!isInputValid(inputText, getDate(), disabledDate)) {
      ...
      // @ts-ignore
      this.setState({ [text]: inputText, [value]: new Date(ndate) }, () => {
  ...
  prevYear(type, date, callback = () => {}) {
    // @ts-ignore
    this.setState({ [type]: prevYear(date) }, callback);
  }

  // 下一年
  nextYear(type, date, callback = () => {}) {
    // @ts-ignore
    this.setState({ [type]: nextYear(date) }, callback);
  }

  // 上个月
  prevMonth(type, date, callback = () => {}) {
    // @ts-ignore
    this.setState({ [type]: prevMonth(date) }, callback);
  }

  // 下个月
  nextMonth(type, date, callback = () => {}) {
    // @ts-ignore
    this.setState({ [type]: nextMonth(date) }, callback);
  }
      ...
      () => {
        ...
        if (type === 'leftDate') {
          if (leftDate >= rightDate) {
            this.setState({
              rightDate: new Date(
                new Date(rightDate).setFullYear(leftDate.getFullYear() + 1),
              ),
            });
          }
          ...
          if (leftDate >= rightDate) {
            this.setState({
              leftDate: new Date(
                new Date(leftDate).setFullYear(rightDate.getFullYear() - 1),
              ),
            });
          }
        }
  ...
  handleChangeMonth(type, date, month) {
    ...
    this.setState(
      {
        [type]: new Date(
          new Date(date).setMonth(parseInt(month.slice(0, -1)) - 1),
        ),
      },
      () => {
  ...
  handleRangePick({ minDate, maxDate }, isClose) {
    ...
    const pickedMinTime = minTime
      ? setTime(new Date(minDate), minTime)
      : minDate;
    const pickedMaxTime = maxTime
      ? setTime(new Date(maxDate), maxTime)
      : maxDate;
    ...
    if (
      ...
      maxDate &&
      onError &&
      onError([pickedMinTime, pickedMaxTime])
    ) {
      return;
    }
    ...
    if (!showTime) {
      ...
      const pickedMinTime = setTime(
        new Date(minDate),
        new Date(new Date().setHours(0, 0, 0, 0)),
      );
      const pickedMaxTime = setTime(
        new Date(maxDate),
    ...
    return (
      ...
      >
          ...
          <div className={`${prefixCls}-picker-panel__body`}>
            {showTime && (
              <div className={`${prefixCls}-date-range-picker__time-header`}>
                <span
                  className={`${prefixCls}-date-range-picker__editors-wrap is-left`}
                >
                  <span
                    className={`${prefixCls}-date-range-picker__time-picker-wrap`}
                  >
                    <Input
                      ...
                      onChange={value =>
                        this.handleDateInputChange(value, 'min')
                      }
                  ...
                  >
                    {startTimeSelectMode === 'TimePicker' ? (
                      <TimePicker
                        ...
                        value={minTime}
                        onChange={value =>
                          this.handleTimeInputChange(value, 'min')
                        }
                ...
                <span
                  className={`${prefixCls}-date-range-picker__editors-wrap is-right`}
                >
                  <span
                    className={`${prefixCls}-date-range-picker__time-picker-wrap`}
                  >
                    <Input
                      ...
                      readOnly={!minDate}
                      onChange={value =>
                        this.handleDateInputChange(value, 'max')
                      }
                  ...
                  >
                    {endTimeSelectMode === 'TimePicker' ? (
                      <TimePicker
                        ...
                        value={maxTime}
                        onChange={value =>
                          this.handleTimeInputChange(value, 'max')
                        }
                        isShowCurrent={showTimeCurrent}
                        selectableRange={endTimeSelectableRange}
                      />
            ...
            <div
              className={`${prefixCls}-picker-panel__content ${prefixCls}-date-range-picker__content is-left`}
            >
              <div className={`${prefixCls}-date-range-picker__header`}>
                <Icon
                  type="left-double"
                  onClick={this.prevYear.bind(
                    this,
                    'leftDate',
                    leftDate,
                    () => {},
                ...
                <YearAndMonthPopover
                  ...
                  onChange={this.handleChangeYear.bind(
                    this,
                    'leftDate',
                    leftDate,
                  )}
                >
                  <span
                    className={`${prefixCls}-date-range-picker__header-label`}
                  >
                ...
                <YearAndMonthPopover
                  ...
                  onChange={this.handleChangeMonth.bind(
                    this,
                    'leftDate',
                    leftDate,
                  )}
                ...
                <Icon
                  type="right-double"
                  onClick={this.nextYear.bind(
                    this,
                    'leftDate',
                    leftDate,
                    this.handleLeftNextYear,
                  )}
                  className={`${prefixCls}-picker-panel__icon-btn ${prefixCls}-date-range-picker__next-btn`}
                />
                <Icon
                  type="right"
                  onClick={this.nextMonth.bind(
                    this,
                    'leftDate',
                    leftDate,
                    this.handleLeftNextMonth,
                  )}
                  className={`${prefixCls}-picker-panel__icon-btn ${prefixCls}-date-range-picker__next-btn`}
                />
              </div>
            ...
            <div
              className={`${prefixCls}-picker-panel__content ${prefixCls}-date-range-picker__content is-right`}
            >
              <div className={`${prefixCls}-date-range-picker__header`}>
                <Icon
                  type="left-double"
        ...
        {typeof footer === 'function' && footer() && (
          <div className={`${prefixCls}-picker-panel__extra-footer`}>
            {footer()}
          </div>
        )}
        {showTime && (
          <div className={`${prefixCls}-picker-panel__footer`}>
            ...
            >
              {Locale.t('datepicker.cancel')}
            </Button>
            <Button
              type="primary"
              className={classNames(`${prefixCls}-picker-panel__btn confirm`, {
